if (Meteor.isServer) {
    ...
    Meteor.startup(function () {
        w2vModel.readTxtFile('D:\\phrases.txt');
    });
}
...
Meteor.methods({
    ...
    synonymNameEntityRecognition: function () {
        console.log('W2V categorisation is running...');
        if (Meteor.isServer) {
            Reports.find({processed: {$ne: 'Categorised'}}).forEach((doc)=> {
                Reports.update(doc._id, {$set: {processed: 'In progress'}});
                var category = Tentacula.synonymNameEntityRecognition(doc.headline, crimes, synomnyms);
                if (category && category.score > 500) {
                    Reports.update(doc._id, {$set: {processed: 'Categorised - unsafe', category: category}});
                    return;
                }

                category = Tentacula.synonymNameEntityRecognition(doc.text, crimes, synomnyms);
                if (category && category.score > 500) {
                    Reports.update(doc._id, {$set: {processed: 'Categorised - unsafe', category: category}});
                    return;
                }
