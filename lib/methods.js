Meteor.methods({
    ...
    reset: function (code) {
        if (Meteor.isServer) {
            if (code === authCode) {
                Reports.update({
                    $or: [{processed: 'Categorised'}, {processed: 'Categorised - unsafe'}]
                }, {
                    $set: {
    ...
    saveNameEntityRecognition: function () {
        if (Meteor.isServer) {
            ...
            Reports.find().forEach((doc)=> {
                ...
                if(Reports.find().count()<=i){
                    console.log('Fuzzy categorisation has finished...');
                }
            });
        }
    },
    synonymNameEntityRecognition: function () {
        if (Meteor.isServer) {
            ...
            Reports.find({processed: {$ne: 'Categorised'}}).forEach((doc)=>{
                ...
                Reports.update(doc._id, {$set: {processed: 'Not categorised'}});
                if(Reports.find({processed: {$ne: 'Categorised'}}).count()<=i){
                    console.log('Fuzzy categorisation has finished...');
                }
            });
        }
